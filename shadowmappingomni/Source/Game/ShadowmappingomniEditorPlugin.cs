...
/// <summary>
/// Imports Shadowmappingomni.shader and glTF models into Content/Models on editor load.
/// </summary>
public class ShadowmappingomniEditorPlugin : EditorPlugin
{
    static readonly string VulkanAssets = Path.GetFullPath(
        Path.Combine(Globals.ProjectFolder, "..", "..", "..", "Vulkan", "assets"));

    static readonly string[] ModelNames = { "shadowscene_fire" };
    ...
    private static void EnsureSourceModels()
    {
        ...
        foreach (var name in ModelNames)
        {
            ...
            {
                Debug.LogWarning($"shadowmappingomni: missing Vulkan asset {name}.gltf");
                continue;
            }
    ...
    private static void RelocateRootImports()
    {
        ...
        foreach (var name in ModelNames)
        {
            ...
            if (File.Exists(rootFlax) && !File.Exists(modelsFlax))
            {
                ...
                Debug.Log($"shadowmappingomni: moved {name}.flax to Content/Models");
            }
            ...
            if (Directory.Exists(rootDir) && !Directory.Exists(modelsDir))
            {
                ...
                Debug.Log($"shadowmappingomni: moved {name}/ materials to Content/Models/{name}");
            }
        }
    }
    ...
    private static void ImportModels()
    {
        ...
        if (folder == null)
        {
            Debug.LogWarning("shadowmappingomni: Content folder not found; cannot import models.");
            return;
        }
        ...
        foreach (var name in ModelNames)
        {
            ...
            Debug.Log($"shadowmappingomni: importing {name}.gltf -> Content/Models/{name}.flax");
        }
    ...
    {
        var sourcePath = Path.Combine(Globals.ProjectFolder, "Source", "Shaders", "Shadowmappingomni.shader");
        if (!File.Exists(sourcePath))
            return;

        var outputDir = Path.Combine(Globals.ProjectContentFolder, "Shaders");
        var outputPath = Path.Combine(outputDir, "Shadowmappingomni.flax");
        ...
        Debug.Log("shadowmappingomni: imported Shadowmappingomni.shader");
    }
}
#endif
